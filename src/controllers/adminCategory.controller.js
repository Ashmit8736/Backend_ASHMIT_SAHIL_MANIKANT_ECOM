  ...
  try {
    ...
    const [result] = await productDB.query(
      `
            ...
            VALUES (?, ?, ?, ?, ?, 1, NOW())
            `,
      [category_name, description || null, image_url || null, parentId, level],
    );
...
export async function adminEnableCategory(req, res) {
  try {
    ...
    await productDB.query(`UPDATE category_master SET status=1 WHERE id=?`, [
      id,
    ]);
...
export async function adminDisableCategory(req, res) {
  try {
    ...
    await productDB.query(`UPDATE category_master SET status=0 WHERE id=?`, [
      id,
    ]);
...
async function deleteCategoryWithChildren(id) {
  ...
  await productDB.query(`DELETE FROM category_master WHERE id = ?`, [id]);
}
export async function adminDeleteCategory(req, res) {
  try {
    ...
    const totalProducts = supplierProducts[0].total + sellerProducts[0].total;
...
export async function adminBulkUploadCategories(req, res) {
  try {
    ...
    await productDB.query("CALL sp_bulk_insert_categories_option1(?)", [
    ...
    res.json({
      success: true,
      message:
        "Categories uploaded successfully. Duplicate entries were skipped.",
    });
  } catch (err) {
